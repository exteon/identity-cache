<?php
    ...
    class IdentityCache extends AIdentityMap implements IIdentityCache {
        protected const TRIGGER_NONE = 0;
        protected const TRIGGER_RETAINED_OBJECTS = 1;
        protected const TRIGGER_SCRIPT_MEMORY = 2;

        protected const PURGE_STRATEGY_RANDOM = 0;
        protected const PURGE_STRATEGY_POPULARITY = 1;

        /** @var array */
        protected $collectionGcHold = [];
         ...
         * @var int[]
         */
        protected $objectsPopularity = [];

        /** @var int[] */
        protected $objectsPopularityDecayUpdated = [];

        /** @var int */
        protected $objectsPopularityDecayCounter = 0;
        ...
        public function offsetGet($offset) {
            ...
            if ($result !== null) {
                ...
                if ($this->purgeStrategy === self::PURGE_STRATEGY_POPULARITY) {
        ...
        protected function checkGcTrigger(): bool {
            switch ($this->trigger) {
                case self::TRIGGER_RETAINED_OBJECTS:
                    return (
                        count($this->collectionGcHold)
                        >
                        $this->config['maxRetainedObjects']
                    );
                case self::TRIGGER_SCRIPT_MEMORY:
                    return (
                        memory_get_usage()
                        >
        ...
        protected function purge(): void {
            switch ($this->purgeStrategy) {
                case self::PURGE_STRATEGY_POPULARITY:
                    ...
                    break;
                case self::PURGE_STRATEGY_RANDOM:
        ...
        public function setConfig(array $config): void {
            ...
            switch ($this->config['trigger']) {
                case 'maxRetainedObjects':
                    $this->trigger = self::TRIGGER_RETAINED_OBJECTS;
                    break;
                case 'maxScriptMemory':
                    $this->trigger = self::TRIGGER_SCRIPT_MEMORY;
                    break;
                case 'none':
                    $this->trigger = self::TRIGGER_NONE;
                    break;
                default:
            ...
            switch ($this->config['purgeStrategy']) {
                case 'popularity':
                    $this->purgeStrategy = self::PURGE_STRATEGY_POPULARITY;
                    ...
                    break;
                case 'random':
                    $this->purgeStrategy = self::PURGE_STRATEGY_RANDOM;
                    break;
                default:
